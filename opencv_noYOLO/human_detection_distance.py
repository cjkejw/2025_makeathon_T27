...
KNOWN_HEIGHT = 1.7  # Average height of a human in meters
FOCAL_LENGTH = 615  # Focal length in pixels (calibrate for accurate results)
    ...
    for result in results:
        for box in result.boxes:  # Each detected box
            ...
            if cls == 0 and confidence > 0.5:  # Class ID 0 = 'person'
                ...
                pixel_height = y2 - y1  # Calculate the height of the bounding box in pixels

                # Estimate distance
                if pixel_height > 0:  # Avoid division by zero
